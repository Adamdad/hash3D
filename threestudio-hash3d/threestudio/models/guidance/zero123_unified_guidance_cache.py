...
import threestudio
from extern.zero123 import Zero123Pipeline as Zero123Pipeline_Base
from DeepCache import Zero123Pipeline as Zero123Pipeline_DC
...
@threestudio.register("zero123-unified-guidance-cache")
class Zero123UnifiedGuidance_Cache(BaseModule):
    @dataclass
    class Config(BaseModule.Config):
        ...
        delta_c: List[float] = field(default_factory=lambda: [20, 20, 0.1])
        N: List[int] = field(default_factory=lambda: [73856093, 19349669, 83492791, 9999953])
        delta_t: int = 10
    ...
    def configure(self) -> None:
        ...
        if self.cfg.cache_mode == "noise":
            Zero123Pipeline = Zero123Pipeline_Base
        elif self.cfg.cache_mode == "features":
            Zero123Pipeline = Zero123Pipeline_DC
        ...
        if self.cfg.hash_table == 'Adaptive':
            self.feature_cache = AdaptiveGridBasedHashTable(self.cfg.delta_c, 
                                                            delta_t=self.cfg.delta_t, N=N, 
                                                            max_queue_length=self.cfg.max_queue_length, 
                                                            hash_table_size=self.cfg.hash_table_size)
        elif self.cfg.hash_table == 'Sim':
            self.feature_cache = GridBasedHashTable_Sim(self.cfg.delta_c, 
                                                            delta_t=self.cfg.delta_t, N=N, 
                                                            max_queue_length=self.cfg.max_queue_length, 
                                                            hash_table_size=self.cfg.hash_table_size)
        elif self.cfg.hash_table == 'Key':
            self.feature_cache = GridBasedHashTable_Key(self.cfg.delta_c, 
                                                            delta_t=self.cfg.delta_t, N=N, 
                                                            max_queue_length=self.cfg.max_queue_length, 
                                                            hash_table_size=self.cfg.hash_table_size)
    ...
    def forward_unet(
        ...
        elevation: Optional[Float[Tensor, "..."]] = None,
        azimuth: Optional[Float[Tensor, "..."]] = None,
        camera_distances: Optional[Float[Tensor, "..."]] = None,
        cache=True,
        cache_layer_id=0,
        cache_block_id=0
    ) -> Float[Tensor, "..."]:
        input_dtype = latents.dtype
        cache_layer_id = self.cfg.cache_layer_id
        cache_block_id = self.cfg.cache_block_id
        ...
        if self.cfg.cache_mode == "noise":
            ...
            if random.random() < self.cfg.cache_p:
                ...
                for i in range(batch_size):
                    ...
                    if prv_noise is None:
                        ...
                        prv_features[i] = prv_noise[0]
                        prv_features[i + batch_size] = prv_noise[1]
            ...
            if append:
                ...
                for i in range(batch_size):
                    ...
                    self.feature_cache.append(key[i], latents[i], t_features)
                
            self.prv_features = prv_features
            
        else:
            AssertionError("Invalid cache mode!")
            

        if velocity_to_epsilon:
            noise_pred = latents * self.sigmas[t].view(-1, 1, 1, 1) + noise_pred * self.alphas[
                t
            ].view(-1, 1, 1, 1)
        return noise_pred.to(input_dtype)
    ...
    @contextmanager
    def set_scheduler(self, pipe, scheduler_class: Any, **kwargs):
