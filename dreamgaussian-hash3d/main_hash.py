...
class GUI:
    ...
    def prepare_train(self):
        ...
        if self.guidance_zero123 is None and self.enable_zero123:
            ...
            self.guidance_zero123 = Zero123(self.device, model_key='ashawkey/stable-zero123-diffusers', cache_p=self.opt.cache_p)
    ...
    def train_step(self, noise=None):
        ...
        for train_step in range(self.train_steps):
            ...
            # render_resolution = 128 if step_ratio < 0.1 else (256 if step_ratio < 0.3 else 512)
            ...
            # guidance loss
            strength = step_ratio * 0.15 + 0.5
            
            # if False:
            # if self.enable_sd and step_ratio > 0.8:
            if self.enable_sd:
                if self.opt.mvdream:
            ...
            if self.enable_zero123:
                ...
                cache = True
                loss = loss + self.opt.lambda_zero123 * self.guidance_zero123.train_step(images, vers, hors, radii, guidance_scale=self.opt.get('guidance_scale', 5.0), cache=cache, noise=noise, step_ratio=step_ratio if self.opt.anneal_timestep else None)
            ...
            if self.opt.grad_clip > 0:
                torch.nn.utils.clip_grad_norm_(self.renderer.gaussians._xyz, self.opt.grad_clip)
                torch.nn.utils.clip_grad_norm_(self.renderer.gaussians._scaling, self.opt.grad_clip)
                torch.nn.utils.clip_grad_norm_(self.renderer.gaussians._rotation, self.opt.grad_clip)
                torch.nn.utils.clip_grad_norm_(self.renderer.gaussians._features_dc, self.opt.grad_clip)
                torch.nn.utils.clip_grad_norm_(self.renderer.gaussians._features_rest, self.opt.grad_clip)
                torch.nn.utils.clip_grad_norm_(self.renderer.gaussians._opacity, self.opt.grad_clip)
    ...
    def train(self, iters=500):
        # noise = torch.randn(1, 4, 32, 32).to(self.device).half()
        noise = None

        if iters > 0:
            self.prepare_train()
            for i in tqdm.trange(iters):
                self.train_step(noise=noise)
