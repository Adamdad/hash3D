...
We propose, Hash3D, a universal solution to acclerate score distillation sampling~(SDS) based 3D generation. By effectively hashing and reusing these feature maps across neighboring timesteps and camera angles, Hash3D substantially prevents redundant calculations, thus accelerating the diffusion model's inference in 3D generation tasks.


## Project Structure
The repository is organized into three main directories, each catering to a different aspect of Hash3D's application:

1. `threesdtudio-hash3d`: Contains the implementation of Hash3D tailored for use with the (`threestudio`)[https://github.com/threestudio-project/threestudio] environment.
2. `dreamgaussian-hash3d`: Focuses on integrating Hash3D with the DreamGaussian for image-to-3D generation.
3. `gaussian-dreamer-hash3d`: Dedicated to applying Hash3D to GaussianDreamer for faster text-to-3D generative tasks.

### What we add?
The core mechanism is in the `guidance_loss` for each SDS loss computation. See `hash3D/threestudio-hash3d/threestudio/models/guidance/zero123_unified_guidance_cache.py` for example. The code for the hash table implementation is in `hash3D/threestudio-hash3d/threestudio/utils/hash_table.py`.
